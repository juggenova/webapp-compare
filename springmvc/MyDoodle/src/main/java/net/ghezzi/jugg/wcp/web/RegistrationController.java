...
import net.ghezzi.jugg.wcp.persistence.entity.WcpRegistrationRequest;
...
public class RegistrationController {
	...
	public String registerPage(WcpRegistrationRequest wcpRegistrationRequest) {
	...
	public String signup(WcpRegistrationRequest wcpRegistrationRequest, BindingResult formBinding, Model model, HttpServletRequest request, Locale locale, RedirectAttributes redirectAttributes) {
		...
		if (StringUtils.isNotEmpty(wcpRegistrationRequest.getUsername())) {
		...
		// Form validation for the fields in the WcpRegistrationRequest subclass only. Superclass fields are validated by Yada
		// No need to check for empty name 
		...
		boolean emailSent = yadaRegistrationController.handleRegistrationRequest(wcpRegistrationRequest, formBinding, model, request, locale);
		
		if (!emailSent || formBinding.hasErrors()) {
			// Go back to the registration form in case of errors
			return registerPage(wcpRegistrationRequest);
		}

		// Redirect to the home page with a confirmation message
		yadaNotify.titleKey(redirectAttributes, locale, "registration.email.confirmed.title").ok().messageKey("registration.email.confirmed.text", wcpRegistrationRequest.getEmail()).add();
	...
	public String registrationConfirmation(@PathVariable String token, Model model, Locale locale, RedirectAttributes redirectAttributes, HttpServletRequest request, HttpSession session) {
		...
		YadaRegistrationOutcome<UserProfile, WcpRegistrationRequest> outcome = yadaRegistrationController.handleRegistrationConfirmation(token, new String[]{config.getRoleName(ROLE_USER_ID)}, locale, session, UserProfile.class, WcpRegistrationRequest.class);
		...
		case OK:
			...
			WcpRegistrationRequest wcpRegistrationRequest = outcome.yadaRegistrationRequest;
			UserProfile userProfile = outcome.userProfile;
			userProfile.setFirstName(wcpRegistrationRequest.getName());
			userProfile.setLastName(wcpRegistrationRequest.getSurname());
